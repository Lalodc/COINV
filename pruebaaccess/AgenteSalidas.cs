...
{
    public partial class AgenteSalidas : Form
    {
        //VARIABLES GLOBALES//
        ...
        private void AgenteSalidas_Load(object sender, EventArgs e)
        {
            //CONTROLAR ERRORES//
            ...
            using (OleDbConnection conect = new OleDbConnection(AgenteSalidas.cadConex))
            {
                ...
                try
                {
                    ...
                    cmd.CommandText = "SELECT TOP 1 IdSalidasAPT FROM [APT - Salidas] ORDER BY IdSalidasAPT desc;";
                    reader = cmd.ExecuteReader();
                    reader.Read();
                    String idAgenteSalida = Convert.ToString(reader.GetValue(0));
                    reader.Close();
                    textBox1.Text = idAgenteSalida;

                    bandera = 0;

                    BuscarSalidaAgenteEncabezado(idAgenteSalida);
                    BuscarSalidaAgenteDetalle(idAgenteSalida);
            ...
            using (OleDbConnection conect = new OleDbConnection(AgenteSalidas.cadConex))
            {
                if (e.KeyChar == Convert.ToChar(Keys.Enter))
                {
                    ...
                    if (Val.IsMatch(textBox1.Text))
                    {
                        ...
                        {
                            bandera = 0;
                            String idAgenteSalida = textBox1.Text;
        ...
        public void BuscarSalidaAgenteEncabezado(String EidAgenteSalida)
        {
            try
            {
                using (OleDbConnection conect = new OleDbConnection(AgenteSalidas.cadConex))
                {
                    ...
                    cmd.CommandText = "SELECT fechasalida, nombrecompleto, nombreregión, usuario, observacionessalida FROM ((([apt - salidas] "
                    +"INNER JOIN agentes ON [apt - salidas].idagente = agentes.idagente) INNER JOIN regiones ON [apt - salidas].idregión = regiones.idregión) "
                    +"INNER JOIN sysvusuario ON [apt - salidas].idelaborósalida = sysvusuario.usuario)  WHERE [apt - salidas].idSalidasAPT = @idAgenteSalida";
                    cmd.Parameters.AddWithValue("@idAgenteSalida", EidAgenteSalida);
        ...
        public void BuscarSalidaAgenteDetalle(String DidAgenteSalida)
        {
            try
            {
                using (OleDbConnection conect = new OleDbConnection(AgenteSalidas.cadConex))
                {
                    ...
                    String query = "SELECT subproductos.idsubproducto, subproductos.nombresubproducto, unidades.descripcion, subproductos.cantidadenvase, "
                        + "[detalle apt - salidas].idlotecaducidad, [detalle apt - salidas].fechacaducidad, [detalle apt - salidas].piezassalidas, [Tipos de cajas].descripcióntipocaja, "
                        + "[detalle apt - salidas].númerodecajassalidas, [detalle apt - salidas].kilossalidas, [Tipos de cajas].pesocaja*[detalle apt - salidas].númerodecajassalidas AS PesoCajas, "
                        + "[detalle apt - salidas].kilossalidas-PesoCajas AS PesoNeto, IIf([detalle apt - salidas].piezassalidas=0, PesoNeto, PesoNeto/[detalle apt - salidas].piezassalidas) as PesoPromedio, "
                        + "[detalle apt - salidas].observacionessalidas FROM subproductos, unidades, [detalle apt - salidas], [Tipos de cajas] "
                        + "WHERE [detalle apt - salidas].IdSalidasAPT = " + DidAgenteSalida + " AND subproductos.idsubproducto = [detalle apt - salidas].idsubproducto "
                        + "AND [Tipos de cajas].idtipocaja = [detalle apt - salidas].IdTipoCaja AND Unidades.IdUnidad = subproductos.idUnidad";
                    ...
                    LlenarGridDeCajasVacias(DidAgenteSalida);
        ...
        public void LlenarGridDeCajasVacias(String DidAgenteSalida)
        {
            try
            {
                using (OleDbConnection conect = new OleDbConnection(AgenteSalidas.cadConex))
                {
                    String query = "SELECT [Tipos de cajas].descripcióntipocaja, [detalle apt - cajassalidas].númerodecajassalidas, "
                        + "[Tipos de cajas].pesocaja*[detalle apt - cajassalidas].númerodecajassalidas as PesoCajas FROM [Tipos de Cajas], [Detalle APT - CajasSalidas] "
                        + "WHERE [Tipos de cajas].idtipocaja = [detalle apt - cajassalidas].IdTipoCaja AND [detalle apt - cajassalidas].idsalidasapt = " + DidAgenteSalida + ";";
        ...
        public void guardarSalidaAgente(OleDbConnection conect)
        {
            try
            {
                conect.Open();
                ...
                //{
                cmd.CommandText = "insert into [APT - Salidas](IdSalidasAPT, FechaSalida, IdAgente, IdRegión, IdElaboróSalida, ObservacionesSalida) "
                ...
                foreach (DataGridViewRow fila in dataGridView1.Rows)
                {
                    ...
                    String IdLoteCaducidadPedidos = fila.Cells[4].Value.ToString().Trim();
                    String FechaCad = fila.Cells[5].Value.ToString().Trim();
                    ...
                    cmd.CommandText = "insert into [Detalle APT - Salidas](IdSalidasAPT, IdSubProducto, IdLoteCaducidad, FechaCaducidad, KilosSalidas, IdTipoCaja, "
                    +"NúmeroDeCajasSalidas, PiezasSalidas, Tara, ObservacionesSalidas) values(@detalleclave, @detalleclavepro, @detallelote, @detallefechacad, @detallepesobruto, "
                    ...
                    cmd.Parameters.AddWithValue("@detallelote", IdLoteCaducidadPedidos);
                    ...
                    cmd.CommandText = "update [detalle existencias apt] set salidasagenteskg = salidasagenteskg + @pesobruto where " +
                ...
                foreach (DataGridViewRow fila2 in dataGridView2.Rows)
                {
                    ...
                    cmd.CommandText = "INSERT INTO [Detalle APT - CajasSalidas](IdSalidasAPT, IdTipoCaja, NúmeroDeCajasSalidas) "
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    if
                        (string.IsNullOrEmpty(row.Cells[0].FormattedValue.ToString()) ||
                         ...
                         string.IsNullOrEmpty(row.Cells[11].FormattedValue.ToString()) ||
                         string.IsNullOrEmpty(row.Cells[12].FormattedValue.ToString()))
                    { bVacia = true; }
                }
            }
            else
