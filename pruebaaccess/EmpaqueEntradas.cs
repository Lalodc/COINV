...
{
    public partial class EmpaqueEntradas : Form
    {
        //VARIABLES GLOBALES//
        ...
        private void EmpaqueEntradas_Load(object sender, EventArgs e)
        {
            //CONTROLAR ERRORES//
            this.dataGridView1.DataError += new DataGridViewDataErrorEventHandler(dataGridView1_DataError);

            //BUSCAR LA ÚLTIMA ENTRADA AL ACCEDER AL FORMULARIO//
            using (OleDbConnection conect = new OleDbConnection(EmpaqueEntradas.cadConex))
            {
                ...
                try
                {
                    ...
                    //BUSCA LA ÚLTIMA ENTRADA//
                    cmd.CommandText = "SELECT TOP 1 IdEAPT FROM [APT-Empaque Entradas] ORDER BY IdEAPT desc;";
                    reader = cmd.ExecuteReader();
                    reader.Read();
                    String idEmpaqueEntrada = Convert.ToString(reader.GetValue(0));
                    textBox1.Text = idEmpaqueEntrada;

                    bandera = 0;

                    BuscarEntradaEmpaqueEncabezado(idEmpaqueEntrada);
                    BuscarEntradaEmpaqueDetalle(idEmpaqueEntrada);
        ...
        //REALIZA LA BUSQUEDA DE LAS ENTRADAS A EMPAQUE AL PRESIONAR ENTER//
            ...
            using (OleDbConnection conect = new OleDbConnection(EmpaqueEntradas.cadConex))
            {
                if (e.KeyChar == Convert.ToChar(Keys.Enter))
                {
                    ...
                    if (Val.IsMatch(textBox1.Text))
                    {
                        ...
                        {
                            bandera = 0;
                            String idEmpaqueEntrada = textBox1.Text;
        ...
        //BUSCAR ENCABEZADO DE LA ENTRADA//
        public void BuscarEntradaEmpaqueEncabezado(String EidEmpaqueEntrada)
        {
            try
            {
                using (OleDbConnection conect = new OleDbConnection(EmpaqueEntradas.cadConex))
                {
                    ...
                    cmd = conect.CreateCommand();
                    //BUSCA LA ENTRADA SEGUN LA CLAVE INGRESADA EN EL TEXTBOX//
                    cmd.CommandText = "SELECT [APT-Empaque Entradas].fechaEAPT, [APT-Empaque Entradas].usuarioentregoeapt, [APT-Empaque Entradas].usuariorecibioeapt " +
                        "from [APT-Empaque Entradas] where [APT-Empaque Entradas].ideapt =" + EidEmpaqueEntrada + ";";                    
                    conect.Open();
                    reader = cmd.ExecuteReader();
                    if (reader.Read())
                    {
        ...
        //BUSCAR DETALLE DE LA ENTRADA//
        public void BuscarEntradaEmpaqueDetalle(String DidEmpaqueEntrada)
        {
            try
            {
                using (OleDbConnection conect = new OleDbConnection(EmpaqueEntradas.cadConex))
                {
                    ...
                    //BUSCAR EL DETALLE DE LA ENTRADA PARA MOSTRARLO EN EL GRID//
                    ///ME QUEDE AQUIIIII/////
                    ///

                    String query = "SELECT subproductos.idsubproducto, subproductos.nombresubproducto, [Detalle APT-Empaque Entradas].idlotecaducidad, "
                        +"[Detalle APT-Empaque Entradas].fechacaducidad, [Detalle APT-Empaque Entradas].piezasrecibidaseapt, [Tipos de cajas].descripcióntipocaja, "
                        +"[Detalle APT-Empaque Entradas].númerodecajas, [Detalle APT-Empaque Entradas].pesobrutoeapt, "
                        +"[Tipos de cajas].pesocaja*[Detalle APT-Empaque Entradas].númerodecajas AS PesoCajas, "
                        +"[Detalle APT-Empaque Entradas].pesobrutoeapt-PesoCajas AS PesoNeto,  "
                        +"IIf([Detalle APT-Empaque Entradas].piezasrecibidaseapt=0, PesoNeto, PesoNeto/[Detalle APT-Empaque Entradas].piezasrecibidaseapt) as PesoPromedio "
                        +"from subproductos, [Detalle APT-Empaque Entradas], [Tipos de cajas] where [Detalle APT-Empaque Entradas].ideapt = " + DidEmpaqueEntrada + " "
                        +"AND subproductos.idsubproducto = [Detalle APT-Empaque Entradas].idsubproducto AND [Tipos de cajas].idtipocaja = [Detalle APT-Empaque Entradas].idtipocaja";
                    /*String query = "SELECT subproductos.idsubproducto, subproductos.nombresubproducto, [Detalle APT-Pedidos Salidas].idlotecaducidadpedidos, " +
                        ...
                        "from subproductos, [Detalle APT-Pedidos Salidas], [Tipos de cajas] where [Detalle APT-Pedidos Salidas].idsaptpedidos =" + DidEmpaqueEntrada + " " +
        ...
        //BOTON NUEVO, LIMPIA EL FORM PARA LLENAR UNA NUEVA ENTRADA//
        ...
        //BUSCA LA ÚLTIMA ENTRADA E INCREMENTA 1, QUE SERÁ LA NUEVA//
        ...
        //BOTON GUARDAR, GUARDA UNA NUEVA ENTRADA//
        ...
        public void guardarEntradaEmpaque(OleDbConnection conect)
        {
            ...
            {
                conect.Open();
                //INICIA LA TRANSACCIÓN PARA GUARDAR LA ENTRADA//
